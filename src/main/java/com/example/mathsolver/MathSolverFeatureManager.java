...
public class MathSolverFeatureManager extends FeatureManager {

	private static final long serialVersionUID = -2563525506924470293L;
	private HybridGrammar g;
	private SemTextDataManager dm;
	private boolean CHEAT = false;

	private enum FEATURE_TYPE {
		emission, transition, pattern, pos, neural, cheat
	};

	public MathSolverFeatureManager(GlobalNetworkParam param_g, HybridGrammar g, SemTextDataManager dm) {
		super(param_g);
		this.dm = dm;
		this.g = g;
	}
	...
	protected FeatureArray extract_helper(Network network, int parent_k, int[] children_k) {
		...
		if (CHEAT) {
			int[] fd = new int[1];
			fd[0] = this._param_g.toFeature(stNetwork, FEATURE_TYPE.cheat.name(), ids_parent[4] + "", "");
			return new FeatureArray(fd);
		}
	...
	private FeatureArray extract_helper(Network network, SemanticUnit p_unit, SemanticUnit[] c_units,
		...
		if (pattern_parent.isw()) {
			...
			// if (MathSolverConfig.USE_PREDICT_NUMBER) {
			...
			// break;
			// }
			// if (pattern_parent.isA() && (eIndex - bIndex) == 1 && isNumber &&
			...
			// return FeatureArray.NEGATIVE_INFINITY;
			// }
			// } else if (MathSolverConfig.USE_GOLD_NUMBER) {
			// boolean isNumber = false;
			// for (int index = bIndex; index < eIndex; index++) {
			...
			if (pattern_parent.isA() && (eIndex - bIndex) == 1 && isNumber && p_unit.getMRL().equals("NUM")) {
				// System.out.println(sent.get(bIndex) + ": " + bIndex);
				// System.out.println("empty");
				return FeatureArray.EMPTY;
			} else if (pattern_parent.isA() && (eIndex - bIndex) == 1 && isNumber && !p_unit.getMRL().equals("NUM")) {
				// System.out.println("infinity");
				return FeatureArray.NEGATIVE_INFINITY;
			}

			// SemTextNetworkConfig._SEMANTIC_PARSING_NGRAM = 1
			for (int historySize = 0; historySize <= MathSolverConfig._SEMANTIC_PARSING_NGRAM - 1; historySize++) {
				int prevWord;
				if (historySize == 0) {
					prevWord = 1;
				} else {
				...
				for (int w = 0; w < prevWord; w++) {
					...
					if (!NetworkConfig.USE_NEURAL_FEATURES || !NetworkConfig.REPLACE_ORIGINAL_EMISSION
						...
						if (MathSolverConfig.USE_POS_FEAT) {
...

		{
			...
			// {
			// return FeatureArray.EMPTY;
			// } else if ((eIndex - bIndex) == 1 && isNumber &&
			// !c_units[0].getMRL().equals("NUM")) {
			// return FeatureArray.NEGATIVE_INFINITY;
			// }
			// } else if (MathSolverConfig.USE_GOLD_NUMBER) {
			// boolean isNumber = false;
			...
			// System.out.println("c_units[0].toString() " + c_units[0].toString());
			...
			if (pattern_parent.isA() && (eIndex - bIndex) == 1 && isNumber && p_unit.getMRL().equals("NUM")) {
				// System.out.println("Empty: " + sent.get(bIndex));
				return FeatureArray.EMPTY;
			} else if (pattern_parent.isA() && (eIndex - bIndex) == 1 && isNumber && !p_unit.getMRL().equals("NUM")) {
				// System.out.println("Infinity: " + sent.get(bIndex));
				return FeatureArray.NEGATIVE_INFINITY;
			}
			// transition -> p_unit -> c_unit
