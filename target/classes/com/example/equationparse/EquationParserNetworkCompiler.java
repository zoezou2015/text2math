...
public class EquationParserNetworkCompiler extends NetworkCompiler {

	private static final long serialVersionUID = 463252654335875857L;
	...
	public EquationParserNetworkCompiler(HybridGrammar g, SemanticForest global_forest, SemTextDataManager dm) {
		this.g = g;
	...
	public SemTextNetwork compile(int networkId, Instance inst, LocalNetworkParam param) {
		...
		EquationInstance stInst = (EquationInstance) inst;
		if (inst.isLabeled()) {
	...
	private SemTextNetwork compile_labeled(int networkId, EquationInstance inst, LocalNetworkParam param) {
	...
	public SemTextNetwork compile_unlabeled(int networkId, Instance inst, LocalNetworkParam param) {
		EquationInstance math_inst = (EquationInstance) inst;
	...
	@Override
	public EquationInstance decompile(Network network) {
	...
	private void toTree_helper(SemTextNetwork network, int node_k, SemanticForestNode currNode) {
		...
		Sentence sent = ((EquationInstance) network.getInstance()).getInput();
		...
		if (this.dm.getAllUnits().get(wIndex).getMRL().equals("VAR")) {
			// System.out.println(this.dm.getAllUnits().get(wIndex).getMRL());
	...
	private double getNumber(Sentence sent, int bIndex, int eIndex) {
		for (int i = bIndex; i < eIndex; i++) {
			WordToken wordToken = sent.get(i);
			double word = wordToken.isNumber() ? wordToken.getNumberVal() : -1.0;
			if (word != -1)
				return word;
		}
